...
// Carousel otimizado para desktop
const VideoCarouselModal: React.FC<VideoCarouselProps> = ({ videos }) => {
  ...
  // Renderizar slides com lógica mobile/desktop separada
  const renderSlides = () => {
    const slides = []
    ...
    if (isMobile) {
      // Comportamento mobile original - apenas 3 slides (prev, current, next)
      for (let i = -1; i <= 1; i++) {
  ...
  return (
    <div className="relative w-full max-w-sm md:max-w-6xl mx-auto py-4 md:py-8">
      ...
      {/* Dica de navegação por teclado - só no desktop */}
